...
public class IDCardUtil {
    /**
     * 1、号码的结构 六位数字地址码，八位数字出生日期码，三位数字顺序码和一位数字校验码。
     ...
     */
    private static int power[] = {7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2};//每位加权因子
    private static String errorInfo = "";// 身份证号错误信息
    ...
    public static boolean IDCardValidate(String IDStr) {
        ...
        if (IDStr.length() == 18) {
            if (!Ai.equals(IDStr) && !AiCache.equals(IDStr)) {
    ...
    public static String getIdcardSex(String idcard) {
        ...
        int ageInt = Integer.valueOf(idcard.substring(16, 17));
        if (1 == ageInt % 2) {
            return "男";
        } else return "女";
    }
    ...
    private static String convertIdcarBy15bit(String idcard) {
        ...
        if (isDigital(idcard)) {
            String year = "19";
            idcard17 = idcard.substring(0, 6) + year + idcard.substring(6, 15);
