...
import com.pickerview.listener.OnDateChangeListener;
...
public class WheelTime {
    ...
    public void setPicker(int year, int month, int day, int h, int m) {
        ...
        wv_year.setAdapter(new NumericWheelAdapter(startYear, endYear, null, context.getString(R.string.pickerview_year)));// 设置"年"的显示数据
//		wv_year.setLabel(context.getString(R.string.pickerview_year));// 添加文字
        wv_year.setCurrentItem(year - startYear);// 初始化时显示的数据

        // 月
        wv_month = (WheelView) view.findViewById(R.id.month);
        wv_month.setAdapter(new NumericWheelAdapter(1, 12, "%02d", context.getString(R.string.pickerview_month)));
//		wv_month.setLabel(context.getString(R.string.pickerview_month));
        ...
        if (list_big.contains(String.valueOf(month + 1))) {
            wv_day.setAdapter(new NumericWheelAdapter(1, 31, "%02d", context.getString(R.string.pickerview_day)));
        } else if (list_little.contains(String.valueOf(month + 1))) {
            wv_day.setAdapter(new NumericWheelAdapter(1, 30, "%02d", context.getString(R.string.pickerview_day)));
        } else {
            // 闰年
            if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0)
                wv_day.setAdapter(new NumericWheelAdapter(1, 29, "%02d", context.getString(R.string.pickerview_day)));
            else
                wv_day.setAdapter(new NumericWheelAdapter(1, 28, "%02d", context.getString(R.string.pickerview_day)));
        }
//		wv_day.setLabel(context.getString(R.string.pickerview_day));
        ...
        OnItemSelectedListener wheelListener_year = new OnItemSelectedListener() {
            final String label = context.getString(R.string.pickerview_day);
            ...
            public void onItemSelected(int index) {
                ...
                if (list_big.contains(String.valueOf(wv_month.getCurrentItem()))) {
                    wv_day.setAdapter(new NumericWheelAdapter(1, 31, "%02d", label));
                    maxItem = 31;
                } else if (list_little.contains(String.valueOf(wv_month.getCurrentItem()))) {
                    wv_day.setAdapter(new NumericWheelAdapter(1, 30, "%02d", label));
                    maxItem = 30;
                } else {
                    if ((year_num % 4 == 0 && year_num % 100 != 0) || year_num % 400 == 0) {
                        wv_day.setAdapter(new NumericWheelAdapter(1, 29, "%02d", label));
                        maxItem = 29;
                    } else {
                        wv_day.setAdapter(new NumericWheelAdapter(1, 28, "%02d", label));
                        maxItem = 28;
                    }
                }
                ...
                if (null != dateChangeListener) {
                    dateChangeListener.onDateChange(getDate());
                }
            }
        };
        // 添加"月"监听
    ...
    public String getDate() {
        StringBuffer sb = new StringBuffer();
        String year = "" + wv_year.getCurrentItem();
        String month = (wv_month.getCurrentItem() > 9 ? "" : "0") + wv_month.getCurrentItem();
        String day = (wv_day.getCurrentItem() > 9 ? "" : "0") + wv_day.getCurrentItem();
